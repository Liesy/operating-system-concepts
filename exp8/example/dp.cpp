...
* 如果建立成功，返回 一个指向该内存区首地址的指针shm_buf * 输入参数：
* shm_key 共享内存的键值
* shm_val 共享内存字节的长度
* shm_flag 共享内存的存取权限
*/
...
dp::dp(int r){
    ...
    Sema *sema;
    rate = r;
    //5个中同时可以有2个在就餐，建立一个初值为2的用于锁的信号灯
