...
pfile: 对应/proc/sysvipc/目录中的 IPC 文件分别为 msg-消息队列,sem-信号量,shm-共享内存
...
/*
set_sem 函数建立一个具有 n 个信号灯的信号量, 如果建立成功,返回 一个信号灯数组的标识符 sem_id
sem_key 信号灯数组的键值
...
/*
set_shm 函数建立一个具有 n 个字节 的共享内存区, 如果建立成功,返回 一个指向该内存区首地址的指针 shm_buf
shm_key 共享内存的键值
shm_val 共享内存字节的长度
shm_flag 共享内存的存取权限
*/
...
/*
set_msq 函数建立一个消息队列, 如果建立成功,返回 一个消息队列的标识符 msq_id
msq_key 消息队列的键值
msq_flag 消息队列的存取权限
*/
